...
export function rankPlayersByLpGain(
  players: { lp: number; prevLp: number; lpGainRank: number }[]
): void {
  let currentRank = 1;
  let previousLpGain = players[0]?.lp - players[0]?.prevLp; // assuming the list is not empty and sorted
  let offset = 0; // to track the number of players with the same lp gain

  for (let i = 0; i < players.length; i++) {
    const currLpGain = players[i].lp - players[i].prevLp;
    if (currLpGain === previousLpGain) {
      // If the lp gain is the same as the previous player, they share the same rank
      players[i].lpGainRank = currentRank;
      offset++;
    } else {
